...
export function UsageModelSettings({ capabilities, sessionId }: Props): ReactElement {
  ...
  return <div className="flex min-h-0 w-full flex-1 flex-col gap-4 px-4" data-testid="settings-usage-model">
    ...
    {sourceTab === "custom" ? <section className="flex min-h-0 flex-1 flex-col gap-3" data-testid="settings-custom-models-panel">
      {providerLoading ? <p>模型加载中...</p> : providerError ? <div role="alert"><span>{providerError}</span><button type="button" onClick={() => void loadProviders()}>重试</button></div> : <>
        ...
        {showProviderForm ? <div role="dialog" aria-modal="true" aria-label={editingProvider ? "编辑提供商" : "添加提供商"} className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4"><form className="flex w-full max-w-[560px] flex-col gap-3 rounded-[12px] bg-bg_default_primary p-5 shadow-lg" onSubmit={(event) => { event.preventDefault(); void saveProvider(); }}>
          <h3 className="text-[16px] font-medium">{editingProvider ? "编辑提供商" : "添加模型"}</h3>
          {!editingProvider && presets.length ? <label className="flex flex-col gap-1">提供商<select aria-label="提供商预设" defaultValue="" onChange={(event) => { const preset = presets.find((item) => item.providerId === event.target.value); if (preset) { setDraftName(text(preset.name)); setDraftBaseUrl(text(preset.baseUrl)); setDraftApiFormat(text(preset.apiFormat)); } }}><option value="">请选择提供商</option>{presets.map((preset) => <option key={text(preset.providerId)} value={text(preset.providerId)}>{text(preset.name)}</option>)}</select></label> : null}
          <label className="flex flex-col gap-1">提供商名称<input value={draftName} onChange={(event) => setDraftName(event.target.value)} required /></label>
          <label className="flex flex-col gap-1">接口地址<input value={draftBaseUrl} onChange={(event) => setDraftBaseUrl(event.target.value)} required /></label>
          <label className="flex flex-col gap-1">API Key<input type="password" value={draftApiKey} onChange={(event) => { setDraftApiKey(event.target.value); setDraftTested(false); }} required={!editingProvider} /></label>
          <label className="flex flex-col gap-1">API 格式<input value={draftApiFormat} onChange={(event) => setDraftApiFormat(event.target.value)} /></label>
          <label className="flex flex-col gap-1">模型名称<textarea value={draftModelId} onChange={(event) => { setDraftModelId(event.target.value); setDraftTested(false); }} required /></label>
          <div className="flex flex-col gap-2 pt-4"><div className="text-[13px] font-medium leading-5 text-text_default_primary">自定义 Headers</div>{draftHeaders.map((header) => <div key={header.id} className="flex items-center gap-2"><input aria-label="Header 名称" className="min-w-0 flex-1" placeholder="Header 名称" value={header.name} onChange={(event) => setDraftHeaders((items) => items.map((item) => item.id === header.id ? { ...item, name: event.target.value } : item))} /><input aria-label="Header 值" className="min-w-0 flex-1" placeholder={header.persistedName ? "留空保留原值" : "Header 值"} value={header.value} onChange={(event) => setDraftHeaders((items) => items.map((item) => item.id === header.id ? { ...item, value: event.target.value } : item))} /><IconButton label="移除 Header" onClick={() => setDraftHeaders((items) => items.filter((item) => item.id !== header.id))}><path d="M9.80273 1.80527C10.605 1.80541 11.2549 2.45615 11.2549 3.2584V4.35606H14.666C14.9422 4.35606 15.166 4.57991 15.166 4.85606C15.1658 5.132 14.942 5.35606 14.666 5.35606H12.8096L12.7861 5.68027L12.3281 11.9645C12.2362 13.2214 11.1891 14.1948 9.92871 14.1949H5.99414C4.72231 14.1949 3.67 13.2045 3.59277 11.9352L3.21387 5.67442L3.19434 5.35606H1.33301C1.05715 5.3559 0.833241 5.1319 0.833008 4.85606C0.833 4.58001 1.057 4.35622 1.33301 4.35606H4.74512V3.2584C4.74512 2.45607 5.39591 1.80527 6.19824 1.80527H9.80273ZM4.21191 5.61387L4.59082 11.8746C4.63603 12.6162 5.25109 13.1949 5.99414 13.1949H9.92871C10.6649 13.1948 11.2761 12.6263 11.3301 11.8922L11.7891 5.60801L11.8076 5.35606H4.19629L4.21191 5.61387ZM6.19824 2.80527C5.9482 2.80527 5.74512 3.00835 5.74512 3.2584V4.35606H10.2549V3.2584C10.2549 3.00844 10.0527 2.80541 9.80273 2.80527H6.19824Z" /></IconButton></div>)}<button type="button" disabled={formBusy} className="mt-1 flex h-8 w-fit items-center gap-1 rounded-[8px] border border-border_default px-2 text-[14px] leading-5 text-text_default_secondary hover:bg-bg_interaction_tertiary_hover disabled:opacity-60" onClick={() => setDraftHeaders((items) => [...items, { id: `new-${Date.now()}-${items.length}`, name: "", value: "" }])}><svg aria-hidden="true" width="18" height="18" viewBox="0 0 18 18" fill="none"><path d="M9 3.5v11M3.5 9h11" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" /></svg>添加 Header</button></div>
          {editingProvider && providers.find((provider) => provider.providerId === editingProvider)?.revision ? <button type="button" disabled={!capabilities.discoverUserModelsCandidate || formBusy} onClick={async () => { const provider = providers.find((item) => item.providerId === editingProvider); try { const result = await capabilities.discoverUserModelsCandidate?.({ providerId: editingProvider, expectedRevision: text(provider?.revision), baseUrl: draftBaseUrl }); if (Array.isArray(result) && result.length) setDraftModelId(result.map((item) => text(record(item).modelId)).filter(Boolean).join("\n")); } catch (error) { setFormError(error instanceof Error ? error.message : String(error)); } }}>自动获取</button> : null}
          <label className="flex items-center gap-2"><input type="checkbox" checked={skipTest} onChange={(event) => setSkipTest(event.target.checked)} />跳过连通检测</label>
          {formError ? <p role="alert">{formError}</p> : null}
          <div className="flex items-center gap-2"><button type="button" disabled={formBusy || !draftModelId.trim()} onClick={() => void runCandidateTest()}>连通检测</button><button type="submit" disabled={formBusy || (!draftTested && !skipTest)}>{formBusy ? "加载中…" : "保存"}</button><button type="button" disabled={formBusy} onClick={() => setShowProviderForm(false)}>取消</button></div>
        </form></div> : null}
